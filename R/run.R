...
#' head(convergence(x))
#' 
#' # We can further use the \code{gene} function to gather all output for a specific gene: gene level, transcript level and convergence results.
#' top_gene = top_genes(x, n = 1)
#' gene(x, top_gene$Gene_id)
#' 
#' # Similarly we can use the \code{transcript} function to gather all output for a specific transcript.
#' top_transcript = top_transcripts(x, n = 1)
#' transcript(x, top_transcript$Transcript_id)
#' 
#' #Finally, we can plot the estimated average transcript relative expression in the two groups for a specific gene via \code{plot_proportions}.
#' library(ggplot2)
...
test_DTU = function(BANDITS_data, prior_precision = NULL, R = 10^4, burn_in = 2*10^3,
                    samples_design, n_cores = 1, gene_to_transcript, theshold_pval = 0.1){
  #########################################################################################################
  ...
  if( !is.data.frame(samples_design) | is.null(samples_design$sample_id) | is.null(samples_design$group) ){
    message("'samples_design' must be a data.frame object with columns 'sample_id' and 'group'")
    return(NULL)
  }
  
  groups = levels(samples_design$group)
  N_groups = length(groups)
  N = sapply( groups, function(g) sum(samples_design$group == g) )
  ...
  ord_samples = unlist(lapply( groups, function(g) which(samples_design$group == g) ) )
  ...
  K_tot_Together = sapply( eff_len_tr_Together, length)
  order_Together = order(K_tot_Together, decreasing = TRUE)
  
  K_tot_Unique = sapply( eff_len_tr_Unique, length)
  ...
  if(N_groups > 2.5){ # if there are 3 or more groups, use the Multi-group test:
    return( test_DTU_multi_group(BANDITS_data = BANDITS_data,
                                 ...
                                 groups = groups, samples_design = samples_design,
  ...
  names(tr_DF)[ seq(7,8) ] = paste("Mean", groups)

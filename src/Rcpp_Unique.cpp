...
double my_det(Rcpp::NumericMatrix mat){
  Function R_det("determinant");
  Rcpp::List L = as<List>(R_det(mat, true));
  return( as<double>(L[0]) );
}

void 
  prior_informative_Rdet(double& prior,
                         Rcpp::NumericVector const& x,
                         double const& mean, 
                         double const& sd,
                         unsigned int const& K){

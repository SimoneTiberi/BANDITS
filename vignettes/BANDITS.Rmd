...
To access the R code used in the vignettes, use:
```{r eval=FALSE} 
...
#### Gene level results {-}
In gene level results, only for two-group comparisons, we also propose a conservative p.value, `p.values_inverted`, which accounts for the inversion of the dominant transcript (i.e., the most expressed transcript).
...
This measure is based on the observation that often differential splicing leads to a change in the dominant transcript and, given the same p.value, it will rank higher genes with different dominant transcritps between conditions.

We also propose a `DTU_measure`, again only defined for two-group comparisons, which is intended to measure the intensity of the DTU change, similarly to fold changes in differential expression analyses.
...
The `DTU_measure` is defined as the absolute difference between the two most expressed transcripts: $\sum_{k \in \tilde{K} } |\pi_k^{(A)} - \pi_k^{(B)}|$, where $\tilde{K}$ indicates the set of two most expressed transcripts across both groups (i.e., adding $\pi_k^{(A)}$ and $\pi_k^{(B)}$).

#### Transcript level results {-}
In transcript level results, `Max_Gene_Tr.p.val` and `Max_Gene_Tr.Adj.p.val` are two conservative transcript level measures which account for both, the gene and transcript level p.values: they are the maximum of gene and transcripts p.values and adjusted p.values, respectively.
With these measures, a transcript can only be detected as significant if the corresponding gene is also significant.

`Mean group_name` and `SD group_name` indicate the posterior mean and standard deviation of each transcript relative abundance.

...
These can be obtained by aligning reads either directly to a reference transcriptome with pseudo-alignmers, such as *salmon* [@salmon], or to a reference genome with splice-aware genome alignment algorithms, such as *STAR* [@STAR], and checking the transcripts compatible with each genome alignment (e.g., via *salmon*).
...
## Load the data
Before loading the data, we compute, via `eff_len_compute`, the median effective length of each transcript (the median is computed with respect to the samples).
```{r}
...
Finally, `Mean group_name` and `SD group_name` indicate the posterior mean and standard deviation of each transcript relative abundance.
